...
namespace AsriATS.Persistance.Repositories
{
    public class ProcessRepository : IProcessRepository
    {
        ...
        public async Task<Process?> GetByIdAsync(int id)
        {
            ...
            var process = await _context.Processes.Include(p => p.WorkflowSequence).ThenInclude(ws => ws.Role).Include(p => p.Requester).Include(p => p.WorkflowActions).Include(p => p.ApplicationJobNavigation).ThenInclude(a => a.JobPostNavigation).ThenInclude(j => j.CompanyIdNavigation).Include(p => p.InterviewScheduleNavigation).SingleOrDefaultAsync(p => p.ProcessId == id);
